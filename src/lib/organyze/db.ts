...
/** Manda várias tarefas para a Lixeira em UMA requisição. */
export async function softDeleteTarefas(supabase: SupabaseClient, ids: string[]): Promise<void> {
...
/** Apaga várias tarefas em UMA requisição (esvaziar lixeira). */
export async function deleteTarefas(supabase: SupabaseClient, ids: string[]): Promise<void> {
	if (!ids.length) return;
	const { error } = await supabase.from('organyze_tarefas').delete().in('id', ids);
	if (error) throw error;
}
