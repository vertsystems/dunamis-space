<script lang="ts">
	import { Card, Badge, toneClasses } from '$lib/components/ui';
	...
	{#each data.notificacoes as n (n.id)}
		...
		>
			...
			<div class="min-w-0 flex-1">
				<div class="text-[0.68rem] uppercase tracking-wide font-semibold text-grey">{n.tipo}</div>
				<div class="text-sm font-medium text-navy">{n.titulo}</div>
				<div class="text-xs text-grey truncate">{n.detalhe}</div>
			</div>
			<span class="self-center shrink-0 text-grey-200"><Icon name="chevron" size={16} /></span>
		</a>
	{:else}
		<p class="px-4 py-12 text-center text-grey">✅ Nenhuma notificação no momento. Tudo em dia!</p>
	{/each}
</Card>
